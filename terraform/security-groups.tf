resource "aws_security_group" "inbound_connections" {
...
resource "aws_security_group_rule" "allow_connections_from_alb" {
  ...
  protocol                 = "tcp"
  security_group_id        = aws_security_group.security_group.id
  source_security_group_id = aws_security_group.inbound_connections.id
}

resource "aws_security_group" "docudb_sg" {
  name        = "docudb_sg"
  description = "Allow EC2 inbound traffic from ECS cluster"
  vpc_id      = aws_vpc.ecs_vpc.id
...
resource "aws_security_group_rule" "allow_ingress_from_ecs" {
  type                     = "ingress"
  from_port                = 0
  to_port                  = 65535
  protocol                 = "tcp"
  security_group_id        = aws_security_group.docudb_sg.id
